...
An implemented component is not proof of 60fps or accessibility across every state. Evidence is revision-specific: PR #4 production passed 36 browser tests; the latest R5 local production-build browser suite has **66 passes, two device-specific skips, zero failures**. The fresh R5 motion harness completed without errors. Production Lighthouse performance remains below the required threshold.
...
| Build passes | Verified | Current R5 production build completed successfully; Queen also reports 357 unit tests passing. Final changes still require the release pipeline. |
| Strict TypeScript and lint | Verified | Latest shared visible-only NYClock candidate passes integrated typecheck, lint and build. Bundle checks pass at **154.1 / 161.7 / 162.3 KiB** for the three sampled initial-script routes, below the 180 KiB budget. |
| Lighthouse performance ≥95 | Blocked | Production remains PR #4 `a118398`: Mac **90**, Linux **69**. Latest R5 Linux CI run **34931595978** recorded fixed five-run scores **75/93/98/95/93**, representative **93** (TBT **205ms**, LCP **2644ms**); the ≥95 gate remains held. After the shared visible-only NYClock fix, local fixed-five scores are **97/98/97/97/97**; computeMedianRun selected **run 3**, score **97**, at 05:19:25.253 UTC (LCP 2626.3609ms, TBT 18ms, CLS 0; other categories 100). Local success does not clear Linux or deployed gates. |
| Lighthouse accessibility ≥95 | Verified | Latest R5 Linux five-run representative audit reports **100** accessibility; production PR #4 also reported 100. The latest local browser suite passes 66 checks with two device-specific skips. |
| Lighthouse best practices 100 | Verified | Latest R5 Linux representative audit and PR #4 production report **100** best practices. Pending R5 production still needs its own audit. |
| Lighthouse SEO 100 | Verified | Latest R5 Linux representative audit and PR #4 production report **100** SEO. Pending R5 production still needs its own audit. |
| Zero browser console errors | Verified | The 36-test PR #4 production run passed. Fresh R5 motion harness recorded no errors. The final R5 browser suite passed 66 tests with two device-specific skips and zero failures, including the existing console-error assertions. |
| All route accessibility, desktop/mobile, JS-off | Verified | Final fresh R5 local production-build browser run: **66 passed, 2 device-specific checks skipped, zero failures**. Synchronous Escape dismissal passed the unchanged regression tests. Earlier PR #4 deployed production run passed 36 tests. |
...
| Mobile and reduced-motion verification | Implemented | Earlier route/motion checks and 36 PR #4 production tests passed their tested scope. Final R5 browser suite passes 66 tests with zero failures; mobile PDF-print and a desktop-only pointer reproduction remain skipped on mobile. Physical devices and full visual/state coverage remain outstanding. |
| Production is measurably as fast or faster | Blocked | Production remains `a118398`, Mac 90/Linux 69. R5 Linux representative performance is 93 and is held below 95. The latest clock fix has local fixed-five representative performance **97** (run 3); no production improvement or Linux pass is claimed. |
...
| 8b. Currently: NYC time, latest commit, latest post | Verified | Shared America/New_York clock now updates only while visible. This fix passes the latest 357-unit/66-browser local suite. Latest captured public commit and Substack article retain six-hour feed refresh; clock changes do not imply fresh external data. |
...
| Live Lighthouse badge updated after every deployment | Implemented | Production audit verifies deployed SHA, uses the same fixed five-run median method as PR CI, and publishes measured scores before enforcing the threshold. Badge reads the dated report; publishing a failing score is not a release pass. Updated workflow execution on a new deployment remains unverified. |
...
| Clean modules, readable source, ingestion tests | Verified | Current R5 full unit run: **357 tests pass**. Seventeen focused feed/renderer tests include safe HTML normalization, source citation preservation, schema validation and refresh/fallback behavior. Readability is not reducible to a test count. |
...
| Measured deployment CI | Implemented | Production audit now uses the same fixed five-run median policy and publishes measured scores before threshold enforcement. Latest R5 PR Linux CI 34931595978 has representative performance 93 from 75/93/98/95/93, so the ≥95 gate remains held. Current deployed PR #4 remains Linux 69. |
...
1. Preserve the final R5 browser result: 66 passed, two device-specific skips, zero failures. Synchronous Escape dismissal is verified by unchanged regression tests; retain the mobile PDF-print limitation explicitly.
2. Preserve the passing build, lint, types, bundle checks and 357 unit tests. Preserve the clock-fix local fixed-five scores 97/98/97/97/97 (selected run 3: 97), then clear **Linux performance ≥95** before release. Latest Linux CI representative is 93; deployed PR #4 remains Linux 69/Mac 90. Verify the final deployed candidate separately.
