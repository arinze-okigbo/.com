...
An implemented component is not proof of 60fps or accessibility across every state. Evidence is revision-specific: PR #4 production passed 36 browser tests; the latest R5 local production-build browser suite has **68 passes, two device-specific skips, zero failures**. The fresh R5 motion harness completed without errors. Production Lighthouse performance remains below the required threshold.
...
| Lighthouse performance ≥95 | Blocked | Production remains PR #4 `a118398`: Mac **90**, Linux **69**. Clock-fix Linux CI **34932388400** recorded fixed-five scores **90/92/97/94/94**, representative **94** (TBT **183ms**, LCP **2658ms**; other categories 100), below the required 95. New candidate local fixed-five scores are **98/98/98/98/98**, representative **98/100/100/100** at **05:35:35 UTC** (LCP **2475.143ms**, TBT **8.5ms**, CLS **0**). This local result does not clear Linux or production gates. |
| Lighthouse accessibility ≥95 | Verified | Latest R5 Linux five-run representative audit reports **100** accessibility; production PR #4 also reported 100. The latest local browser suite passes 68 checks with two device-specific skips. |
...
| Zero browser console errors | Verified | The 36-test PR #4 production run passed. Fresh R5 motion harness recorded no errors. The final R5 browser suite passed 68 tests with two device-specific skips and zero failures, including the existing console-error assertions. |
| All route accessibility, desktop/mobile, JS-off | Verified | Final fresh R5 local production-build browser run: **68 passed, 2 device-specific checks skipped, zero failures**. Synchronous Escape dismissal passed the unchanged regression tests. Earlier PR #4 deployed production run passed 36 tests. |
...
| Production is measurably as fast or faster | Blocked | Production remains `a118398`, Mac 90/Linux 69. Clock-fix Linux representative performance is 94 and held below 95. New candidate local fixed-five performance is 98 across all five runs; neither a Linux pass nor production improvement is claimed. |
...
| Measured deployment CI | Implemented | Production audit uses the same fixed five-run median policy and publishes measured scores before threshold enforcement. Latest clock-fix Linux CI 34932388400 has representative performance 94 from 90/92/97/94/94, so the ≥95 gate remains held. Current deployed PR #4 remains Linux 69. |
...
2. Preserve the passing build, lint, types, bundle checks, 358 unit tests and 68 browser checks. Preserve the new candidate’s local fixed-five 98/98/98/98/98 result, then clear **Linux performance ≥95** before release. Latest clock-fix Linux representative is 94; deployed PR #4 remains Linux 69/Mac 90. Verify the final deployed candidate separately.
